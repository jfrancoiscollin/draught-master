...
function formatScore(score: number): string {
  const v = score / 100
  if (Math.abs(v) >= 90) return score > 0 ? '+♛' : '-♛'
  if (v === 0) return '0.00'
...
export default function ScanBar({ info, loading = false }: ScanBarProps) {
  ...
  useEffect(() => {
    if (info) return  // engine responded, stop counting
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-950 border-b border-gray-800 text-xs font-mono select-none min-h-[28px]">
      ...
      <span className={`text-[10px] ${
        !info ? 'text-yellow-500 animate-pulse' :
        isRunning ? 'text-green-400 animate-pulse' : 'text-green-600'
      }`}>●</span>
      ...
      {!info && (
        <span className="text-gray-600 font-sans">{loadingLabel}</span>
      )}
